...
$ctrlForm .= "
    private function ValidarDatos(): bool {
        ...
        return intval(\$_GET['id'] ?? 0) === \$this->$pk;
    }
...
// Generar template completo con enlaces corregidos
$viewList = <<<HTML
<h1>Listado {$Clase}</h1>
...
<section class="grid row">
    <form class="depth-0 offset-3 col-6"
        action="index.php?page=Mantenimientos-{$Clase}-Formulario&mode={{mode}}&id={{id}}" method="POST">

        <div class="row my-3 align-center">
            <div class="col-4"><label for="id">ID</label></div>
            <div class="col-8">
                <input type="text" id="id" value="{{id}}" disabled name="idux" />
